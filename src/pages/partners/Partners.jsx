...
import { listSlumsoccerPartners, getSlumsoccerPartners } from "../../graphql/queries";
import { deleteSlumsoccerPartners } from "../../graphql/mutations";
import {
  CircularProgress,
  Button,
  Dialog,
...
export function Partners() {
  ...
  const fetchPartners = async () => {
    try {
      ...
      const partnerItems = partnersData.data.listSlumsoccerPartners.items;
      setPartners(partnerItems);
      
      // Extract unique partnership types for filtering
      const types = [...new Set(partnerItems.map(partner => partner.partnershipType))].filter(Boolean);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await client.graphql({
        query: deleteSlumsoccerPartners,
        variables: {
          input: {
      ...
      // Remove deleted partner from state
      setPartners(partners.filter(p => p.partnerId !== selectedPartner.partnerId));
      setNotification({
        open: true,
  ...
  const filteredPartners = partners
    .filter(partner => !filterType || partner.partnershipType === filterType)
    .filter(partner => 
      searchTerm === "" || 
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <h1 className="text-2xl font-bold mb-4 md:mb-0">Partners</h1>
                  ...
                  <tr key={partner.partnerId} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                          ...
                          <div className="h-10 w-10 rounded-md overflow-hidden mr-3">
                            <img 
                              ...
                              onError={(e) => {
                                ...
                                e.target.src = "https://via.placeholder.com/40?text=Partner";
                              }}
                            />
                          </div>
                        ) : (
                        ...
                        <div className="font-medium text-gray-900">{partner.title}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                        {partner.partnershipType || "Not specified"}
                      </span>
                    </td>                    
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        <Tooltip title="Edit">
                          <Link to={`/partners/edit/${partner.partnerId}`}>
